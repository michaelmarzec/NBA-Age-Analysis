...
bucket = s3.Bucket('nba-age-analysis') # Enter your bucket name, e.g 'Data'
file_key = 'age_tracking.csv'
...
# Data Export #
def data_tracking_export(df, file_name = '/tmp/age_tracking.csv', team_var = 'Team_Name', date_var = 'date'):
...
def main(main_fileName):
	...
	data_tracking_export(return_df, main_fileName)

######### lambda ########## 

def lambda_handler(event, context):
    
    local_file_name = '/tmp/age_tracking.parquet' #
    bucket.download_file(file_key,local_file_name)
    
    main(local_file_name)

    bucket.upload_file(local_file_name, file_key)
